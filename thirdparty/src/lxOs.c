...
static struct lx_memery_Obj * gAlloctor;

void setAlloctor(struct lx_memery_Obj * alloctor)
{
    gAlloctor = alloctor;
}
...
void * lxOSMalloc(size_t st)
{
    ...
    void * mOSMalloPtr = NULL;

    if(gAlloctor != NULL)
    {
        mOSMalloPtr = MALLOC(gAlloctor, st);
    }
    else
    {
        mOSMalloPtr = malloc(st);
    }
